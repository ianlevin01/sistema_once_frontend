...
import {
  ...
  searchCustomers, searchProveedores, getWarehouses, getLastSalePrice,
} from "../utils/api";
              ...
              {esReposicion ? (
                provSel ? (
                  <div style={{ background:"var(--accent-dim)", border:"1px solid var(--accent)", borderRadius:6, padding:"10px 12px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                    ...
                    <button onClick={() => setProvSel(null)} style={{ background:"none", border:"none", color:"var(--accent)", cursor:"pointer", fontSize:16 }}>✕</button>
                  </div>
                ) : (
                  <>
                    <div className="search-bar">
                      <span className="search-icon">🔍</span>
                      <input placeholder="Nombre o CUIT..." value={provQuery} onChange={(e) => setProvQuery(e.target.value)} style={{ fontSize:13 }} />
                    </div>
            ...
            <div style={{ flex:1, overflowY:"auto", padding:"16px 18px", borderBottom:"1px solid var(--border)" }}>
              {esReposicion && (
                <div className="input-group">
                  <label className="input-label">Depósito de destino</label>
                  <select className="select" value={warehouseId} onChange={(e) => setWarehouseId(e.target.value)} style={{ fontSize:13 }}>
              ...
              <div className="input-group">
                ...
                <input className="input" value={textoLibre} onChange={(e) => setTextoLibre(e.target.value)} placeholder="Texto libre..." style={{ fontSize:13 }} />
              </div>
            </div>
            ...
            <div style={{ borderTop:"2px solid var(--border)", background:"var(--bg2)", padding:"14px 28px 16px", flexShrink:0 }}>
              {prodSel && (
                <div style={{ marginBottom:10 }}>
                  <div style={{ padding:"9px 14px", background:"var(--accent-dim)", border:"1px solid var(--accent)", borderRadius:6, display:"flex", alignItems:"center", gap:12 }}>
                  ...
                  {lastPrice && custSel && (
                    <div style={{ marginTop:6, padding:"6px 12px", background:"rgba(255,200,0,0.08)", border:"1px solid rgba(255,200,0,0.25)", borderRadius:5, fontSize:12, fontFamily:"var(--font-mono)", color:"var(--text-muted)", display:"flex", gap:12, alignItems:"center" }}>
                      ...
                      <span style={{ color:"var(--accent)", fontWeight:700 }}>${Number(lastPrice.unit_price).toLocaleString("es-AR")}</span>
              ...
              <div style={{ display:"flex", gap:10, alignItems:"center", marginBottom:10 }}>
                ...
                <input className="input" style={{ flex:1, fontSize:14, height:36 }} placeholder="Enter si no modifica"
